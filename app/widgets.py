...
# ---------------------------------------------------------------- 壁纸层（经典 WorkerW 方案）
"""经典动态壁纸嵌入：向 Progman 发 0x052C 生成壁纸层 WorkerW，
应用窗口 SetParent 到该系统 WorkerW（跨进程父）。
系统 WorkerW 是普通顶层窗口（有重定向表面），子窗口 GDI 内容可正常合成
（区别于 Progman 无表面导致的黑块）。Win+D 时 Progman 树保留，应用不消失。"""


def get_wallpaper_workerw() -> int:
    """获取系统壁纸层 WorkerW 句柄（0=失败）。
    1) Raised desktop（Win11）：Progman 的子 WorkerW（壁纸层）；
    2) 传统模式：含 SHELLDLL_DefView 的顶层窗口的下一个 WorkerW（Z 序其后）。"""
    try:
        user32 = _user32()
        progman = user32.FindWindowW("Progman", None)
        if progman:
            out = ctypes.c_ulong()
            user32.SendMessageTimeoutW(wintypes.HWND(progman), 0x052C,
                                       0xD, 0x1, _SMTO_NORMAL, 1000,
                                       ctypes.byref(out))
        if progman:
            ww = user32.FindWindowExW(wintypes.HWND(progman), None,
                                      "WorkerW", None)
            if ww and user32.IsWindowVisible(wintypes.HWND(ww)):
                return int(ww)
        result = [0]
        ...
        def _cb(hwnd, _lp):
            if result[0]:
                return True
            defview = user32.FindWindowExW(wintypes.HWND(hwnd), None,
                                           "SHELLDLL_DefView", None)
            if defview:
                ww = user32.FindWindowExW(None, wintypes.HWND(hwnd),
                                          "WorkerW", None)
                if ww:
                    result[0] = int(ww)
            return True
        ...
        log.error("get_wallpaper_workerw 异常:\n" + traceback.format_exc())
        return 0


def embed_to_sys_workerw(win: QWidget, workerw: int) -> bool:
    """把应用窗口嵌入系统壁纸层 WorkerW（跨进程父，Qt 侧保持顶层语义）。
    WorkerW 全屏(0,0)=屏幕坐标，Qt 内部几何直接对应（无需坐标转换）。
    关闭窗口级半透明：子窗口用 GDI 渲染（内容合成到 WorkerW 重定向表面）。"""
    try:
        from PySide6.QtCore import Qt as _Qt
        user32 = _user32()
        win.setAttribute(_Qt.WA_TranslucentBackground, False)
        hwnd = wintypes.HWND(int(win.winId()))
        style = user32.GetWindowLongPtrW(hwnd, _GWL_STYLE)
        if not style & _WS_CHILD:
            user32.SetWindowLongPtrW(hwnd, _GWL_STYLE, style | _WS_CHILD)
        user32.SetParent(hwnd, wintypes.HWND(workerw))
        user32.SetWindowPos(hwnd, wintypes.HWND(0),
                            win.x(), win.y(),
                            win.width(), win.height(), 0x0010)  # SWP_NOACTIVATE
        user32.ShowWindow(hwnd, 5)
        return True
    except Exception:
        log.error("embed_to_sys_workerw 异常:\n" + traceback.format_exc())
        return False
