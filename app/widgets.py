...
# ---------------------------------------------------------------- 壁纸层 WorkerW（同进程子窗口）
"""壁纸化（三原则·桌面化）：自建 WorkerW 子窗口挂入 Progman，应用嵌入其中。
Win+D 显示桌面时 Progman 树保留，应用不消失；同进程 SetParent 规避
跨进程父 + Qt 分层窗口(WA_TranslucentBackground)失效导致窗口被摧毁的问题。
结构：Progman → [ZhaxxWorkerW(壁纸层)] → 应用窗口
    ...
    try:
        ...
        if _workerw_class is None:
            _workerw_wndproc_cb = ctypes.WINFUNCTYPE(
                ...
                wintypes.WPARAM, wintypes.LPARAM)(_workerw_wndproc)
            cls = _WNDCLASSW()
            ...
            _workerw_class = user32.RegisterClassW(ctypes.byref(cls))
            if not _workerw_class:
                _workerw_class = 1  # 已注册过
        hwnd = user32.CreateWindowExW(
            0x80,  # WS_EX_TOOLWINDOW：不出现在任务栏/Alt-Tab
            "ZhaxxWorkerW", "", 0x80000000 | 0x10000000,  # WS_POPUP | WS_VISIBLE
            x, y, w, h, None, None, hinst, None)
        return int(hwnd or 0)
    except Exception:
        log.error("create_workerw 异常:\n" + traceback.format_exc())
        return 0


def attach_workerw_to_desktop(workerw: int) -> bool:
    """把壁纸层挂入 Progman 并提到图标层之上（应用区域内的点击由应用处理）。"""
    try:
        user32 = _user32()
        progman = user32.FindWindowW("Progman", None)
        if not progman:
            return False
        user32.SetParent(wintypes.HWND(workerw), wintypes.HWND(progman))
        user32.SetWindowPos(wintypes.HWND(workerw), wintypes.HWND(0), 0, 0, 0, 0,
                            _SWP_NOSIZE | _SWP_NOMOVE | _SWP_NOACTIVATE)
        return True
    except Exception:
        log.error("attach_workerw_to_desktop 异常:\n" + traceback.format_exc())
        return False
...
def move_workerw_with(workerw: int, x: int, y: int, w: int, h: int):
    """应用移动/缩放时同步壁纸层位置与尺寸（保持 1:1 跟随）。"""
    try:
        user32 = _user32()
        user32.SetWindowPos(wintypes.HWND(workerw), wintypes.HWND(0),
                            x, y, w, h, _SWP_NOACTIVATE)
    except Exception:
        pass


def unembed_from_workerw(win: QWidget, workerw: int):
    """解除壁纸嵌入：应用还原为顶层窗口，壁纸层销毁。"""
    try:
        user32 = _user32()
        hwnd = wintypes.HWND(int(win.winId()))
        user32.SetParent(hwnd, wintypes.HWND(0))
        ...
        user32.DestroyWindow(wintypes.HWND(workerw))
    except Exception:
        pass
