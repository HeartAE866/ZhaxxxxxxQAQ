...
from widgets import (FramelessDialog, ColorDialog, ConfirmDialog, HotkeyEdit,
                     Toast, combo_text, rounded_pixmap, BgFrame)
...
class SettingsWindow(FramelessDialog):
    ...
    def __init__(self, app):
        super().__init__(None, app.config.get("theme_settings"), "设置", width=790)
    ...
    def _page_personal(self):
        ...
        self.nav.addItem("🎨 个性化")
        self.pages.addWidget(scroll)

        # 美化对象：桌面应用主题 / 设置栏主题（按钮式，选中高亮明显）
        trow0 = QHBoxLayout()
        trow0.addWidget(QLabel("美化对象"))
        self.rb_main_theme = QPushButton("🖥 桌面应用主题", checkable=True,
                                         objectName="ThemeToggle")
        self.rb_settings_theme = QPushButton("⚙ 设置栏主题", checkable=True,
                                             objectName="ThemeToggle")
        ...
        # 各部件背景
        dlab = QLabel("各部件背景（纯色或图片，留空=透明显示下层背景）")
        ...
        # 顶部时钟显示
        crow = QHBoxLayout()
        self.show_clock_chk = QCheckBox("显示时钟（底部日期时间）")
    ...
    def _rebuild_diy_grid(self):
        ...
        for i, (key, name) in enumerate(self._diy_components()):
            ...
            sp = QSlider(Qt.Horizontal, minimum=0, maximum=100, value=alpha,
                         fixedWidth=110)
            sp.setToolTip(f"部件背景不透明度：{alpha}")
            sp.valueChanged.connect(
                lambda v, k=key, s=sp: (s.setToolTip(f"部件背景不透明度：{v}"),
    ...
    def _save_theme(self):
        ...
        name, ok = QInputDialog.getText(self, "保存主题", "主题名称：",
                                        text=self._edit.get("name", "自定义主题"))
    ...
    def _import_theme(self):
        path, _ = QFileDialog.getOpenFileName(self, "导入主题", core.EXPORT_DIR,
                                              "主题文件 (*.json)")
        if path:
            try:
                ...
                self.app.apply_theme(self._kind)
                self._sync_controls()
                Toast.show_text("主题已导入")
            except Exception as e:
                core.log.error(f"导入主题失败: {e}")
                Toast.show_text("主题文件无效")
    ...
    def _page_folder(self):
        ...
        lay = QVBoxLayout(w)
        lay.setAlignment(Qt.AlignTop)
        self.nav.addItem("📁 文件夹")
        self.pages.addWidget(w)

        lay.addWidget(QLabel("父目录（所有自动生成的文件夹都在这里）"))
        row = QHBoxLayout()
        ...
        # ---- 文件夹生成规则树
        lay.addSpacing(10)
        lay.addWidget(QLabel("文件夹生成规则（新建事项自动按此结构创建文件夹）"))
        self.rule_tree = QTreeWidget()
        self.rule_tree.setColumnCount(3)
        self.rule_tree.setHeaderLabels(["结构", "命名模板", "示例"])
        ...
        rtip = QLabel("模板占位符：{Y}=年份  {M}=月份  {D}=日  {name}=事项名称"
                      "（层级可增删、上下移、改名）")
        ...
        # ---- 快捷自定义子文件夹（可选直接绑定）
        lay.addSpacing(10)
        lay.addWidget(QLabel("快捷自定义子文件夹（可选：可直接把事项绑定到这些文件夹）"))
    ...
    def _reload_rule_tree(self):
        ...
        for i, lvl in enumerate(levels):
            ...
            it = QTreeWidgetItem([f"第{i + 1}层", "", ex])   # 第2列文字由内嵌编辑框显示
            root.addChild(it)
            it.setExpanded(True)
            edit = QLineEdit(tpl)
    ...
    def _save_folder_rules(self):
        if not getattr(self, "_rule_levels", None):
            return
        self.app.config.set("folder_rules", {"levels": self._rule_levels})
    ...
    def _add_custom_folder(self):
        from PySide6.QtWidgets import QInputDialog
        name, ok = QInputDialog.getText(self, "新建子文件夹", "子文件夹名称：")
        ...
        Toast.show_text("子文件夹已创建")

    def _rename_custom_folder(self):
        from PySide6.QtWidgets import QInputDialog
        old = self._selected_custom_folder()
        if not old:
            Toast.show_text("请先选择一个子文件夹")
            return
        name, ok = QInputDialog.getText(self, "重命名", "新的名称：", text=old)
    ...
    def _remove_custom_folder(self):
        ...
        if not name:
            Toast.show_text("请先选择一个子文件夹")
            return
        ok, _ = ConfirmDialog.ask(self, self.t, "移除子文件夹",
                                  f"从列表中移除「{name}」？\n（不会删除磁盘上已存在的内容）",
                                  ok_text="移除")
        if not ok:
            return
    ...
    def _page_hotkey(self):
        ...
        lay = QVBoxLayout(w)
        lay.setAlignment(Qt.AlignTop)
        self.nav.addItem("⌨ 快捷键")
        self.pages.addWidget(w)

        lay.addWidget(QLabel("点击输入框后按下组合键（支持四键组合），全部松开即保存"))
    ...
    def _page_data(self):
        w = QWidget()
        lay = QVBoxLayout(w)
        self.nav.addItem("🗄 数据管理")
        self.pages.addWidget(w)

        # 搜索 + 筛选
        row = QHBoxLayout()
        self.data_search = QLineEdit(placeholderText="🔍 搜索全部事项…")
        ...
        # 标签管理
        lay.addWidget(QLabel("标签管理（双击从所有事项中移除）"))
    ...
    def _reload_data_list(self):
        ...
        for it in self.app.store.items:
            ...
            if it["type"] == "recur":
                ...
                state = "（已完成）"
            text = f"[{core.TYPE_NAMES[it['type']]}] {it['title']} {state} " \
                   f"· {it.get('created', '')}"
    ...
    def _reload_recur_history(self):
        ...
        if it:
            ...
            if not it.get("completed_instances"):
                self.recur_history.addItem("（暂无完成记录）")
    ...
    def _export(self, all_items):
        ...
        if not items:
            Toast.show_text("没有可导出的事项")
            return
        from PySide6.QtWidgets import QInputDialog
        fmt, ok = QInputDialog.getItem(self, "导出格式", "选择格式：",
                                       ["JSON", "CSV"], 0, False)
        if not ok:
            return
        path = core.export_items(items, fmt.lower())
        Toast.show_text("已导出到 导出 目录")
        core.open_folder(core.EXPORT_DIR)

    def _import(self):
        path, _ = QFileDialog.getOpenFileName(self, "导入数据", core.EXPORT_DIR,
                                              "数据文件 (*.json *.csv)")
        if not path:
            return
        try:
    ...
    def _do_import(self, new_items):
        ...
        if dups:
            ok, overwrite = ConfirmDialog.ask(
                self, self.t, "发现重复",
                f"导入的 {len(new_items)} 条中有 {len(dups)} 条与现有数据重复。\n"
                f"勾选下方复选框后确定 = 覆盖重复项；不勾选 = 跳过重复项。",
                checkbox="覆盖重复项", ok_text="开始导入")
            if not ok:
                return
        added = 0
        for it in new_items:
        ...
        Toast.show_text(f"导入完成：新增 {added} 条")

    def _delete_selected(self):
        items = self._selected_items()
        if not items:
            Toast.show_text("请先在列表中选择要删除的事项")
            return
        bound = [i for i in items if i.get("folder")]
        msg = f"即将删除 {len(items)} 条事项：\n" + \
              "\n".join(f"· {i['title']}" for i in items[:8])
        if len(items) > 8:
            msg += f"\n…等共 {len(items)} 条"
        checkbox = None
        if bound:
            msg += f"\n\n其中 {len(bound)} 条已绑定工作文件夹。"
            checkbox = "同时删除绑定的工作文件夹（不可恢复！）"
        ok, del_folder = ConfirmDialog.ask(self, self.t, "确认删除", msg,
                                           checkbox=checkbox, ok_text="删除")
        if not ok:
            return
        if del_folder:
            for i in bound:
        ...
        self._reload_data_list()
        self.app.win.refresh()
        Toast.show_text(f"已删除 {len(items)} 条")

    def _remove_tag(self, lw_item):
        tag = lw_item.text()
        ok, _ = ConfirmDialog.ask(self, self.t, "移除标签",
                                  f"将从所有事项中移除标签「{tag}」？", ok_text="移除")
        if ok:
            for it in self.app.store.items:
    ...
    def _page_logs(self):
        ...
        self.log_combo.addItems(["运行日志 app.log", "错误日志 error.log"])
    ...
    def _reload_log(self):
        ...
        if os.path.exists(path):
            try:
                ...
                text = f"读取失败: {e}"
        self.log_view.setPlainText(text or "（暂无日志）")
    ...
    def _page_about(self):
        ...
        lay.addWidget(title)
        lay.addWidget(QLabel("轻量桌面工作记事录 · 完全本地离线"))
        lay.addSpacing(6)
        lay.addWidget(QLabel("作者：你的好邻居\n联系邮箱：1559573443@qq.com\nQQ：1559573443"))
        # 打赏作者（下移三行，与上方信息隔开）
        lay.addSpacing(45)
        donate = QLabel("打赏作者")
        ...
        if os.path.exists(core.DONATION_IMG):
            ...
            donate_img.setToolTip("微信/支付宝扫码打赏，感谢支持！")
        ...
        btxt = QVBoxLayout()
        btxt.setSpacing(2)
        tip = QLabel("所有数据保存在程序目录内，卸载只需删除本地文件夹。",
        ...
        btxt.addWidget(tip)
        warn = QLabel("本应用完全开源且免费，任何向你收费的都是骗子。",
                      alignment=Qt.AlignHCenter)
